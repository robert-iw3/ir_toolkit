...
That context is the disambiguator. A real implant looks like `region=anon`, `perms=rwx`, `path=""`, with a _specific_ matched string - injected, unbacked, executable code. A false positive looks like a rule grazing the bytes of a legitimate on-disk file. From the live `reports/ubuntu-main` run:
