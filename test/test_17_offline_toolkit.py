...
from conftest import ROOT, IRCOLLECT_PS1, IRCOLLECT_SH, read_text
...
def test_linux_builder_accounts_for_memory_and_cloud():
    ...
    assert "avml" in src.lower()                 # Linux memory acquisition
    assert "avml-convert" in src                 # decompress --compress LiME
...
def test_linux_builder_check_only_writes_manifest(tmp_path):
    ...
    assert {"avml-convert", "dwarf2json", "volatility3-wheels", "yara_rules"} <= names
...
def test_memory_capture_wired_both_platforms():
    assert "-CaptureMemory" in read_text(IRCOLLECT_PS1)       # Windows (winpmem)
    assert "tools\\winpmem.exe" in read_text(IRCOLLECT_PS1)
