...
def _score(data: bytes) -> int:
    ...
    if _KEYLOG_RE.search(data):
        score += 2
    if _LICENSE_RE.search(data):
        score += 1
    # SMTP cluster: host + port + email within 512-byte window
    smtp_hosts = list(_SMTP_HOST_RE.finditer(data))
    if smtp_hosts:
        for mh in smtp_hosts:
...
class AgentTeslaConfig(mwcp.Parser):
    ...
    def identify(cls, file_object) -> bool:
        ...
        if len(data) < 64:
            return False
        return _score(data) >= 2

    def run(self):
        data = self.file_object.data
        if not data or _score(data) < 2:
            return
