...
# Both UTF-8 and UTF-16LE variants. \b word boundaries are load-bearing --
# without them these substrings match inside unrelated Windows API export
# names (e.g. "Group" inside "TpReleaseCleanupGroup", "Mutex" inside
# "CreateMutexA"/"OpenMutexA"), which are present in virtually every
# process's mapped kernelbase.dll/ntdll.dll strings and produced a wall of
# false positives against real memory captures.
_STR_CLUSTER_RE = re.compile(
    rb'\b(?:Hosts|Ports|Version|Mutex|Certificate|Pastebin|BDOS|Group|Delay|Install)\b',
    re.IGNORECASE
)
...
_HOSTS_AFTER_RE = re.compile(
    rb'(?:H\x00o\x00s\x00t\x00s\x00|\bHosts\b)[\x00-\x08]{0,8}([a-zA-Z0-9\.\-]{3,253}(?:,[a-zA-Z0-9\.\-]{3,253})*)',
    re.IGNORECASE
)
_PORTS_AFTER_RE = re.compile(
    rb'(?:P\x00o\x00r\x00t\x00s\x00|\bPorts\b)[\x00-\x08]{0,8}(\d{2,5}(?:,\d{2,5})*)',
    re.IGNORECASE
)
_MUTEX_AFTER_RE = re.compile(
    rb'(?:M\x00u\x00t\x00e\x00x\x00|\bMutex\b)[\x00-\x08]{0,8}([A-Za-z0-9_\-\{\}]{4,80})',
    re.IGNORECASE
)
_VERSION_AFTER_RE = re.compile(
    rb'(?:V\x00e\x00r\x00s\x00i\x00o\x00n\x00|\bVersion\b)[\x00-\x08]{0,8}(\d+\.\d+(?:\.\d+)?)',
    re.IGNORECASE
)
_GROUP_AFTER_RE = re.compile(
    rb'(?:G\x00r\x00o\x00u\x00p\x00|\bGroup\b)[\x00-\x08]{0,8}([A-Za-z0-9_\-\.]{1,60})',
    re.IGNORECASE
)
...
class AsyncRATConfig(mwcp.Parser):
    ...
    def identify(cls, file_object) -> bool:
        ...
        # The key cluster (3+ distinct AsyncRAT resource-key names, word-
        # bounded, within a 4KB window) is the actual signal.
        return _check_key_density(data)
