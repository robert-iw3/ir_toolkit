...
# Cluster of Quasar-specific resource key names
_CLUSTER_RE = re.compile(
    rb'(?:Hosts|Port(?!s)|Password|Mutex|Tag|ShowConsole|LogDirectoryName|Version|InstallName)',
    re.IGNORECASE
)
_MIN_KEY_HITS = 3
_WINDOW_SIZE  = 4096
...
_HOSTS_RE   = re.compile(
    rb'Hosts[\x00-\x08]{0,8}([a-zA-Z0-9\.\-]{3,253}(?:;[a-zA-Z0-9\.\-]{3,253})*)',
    re.IGNORECASE
)
_PORT_RE    = re.compile(
    rb'Port(?!s)[\x00-\x08]{0,8}(\d{2,5})',
    re.IGNORECASE
)
_MUTEX_RE   = re.compile(
    rb'Mutex[\x00-\x08]{0,8}([A-Za-z0-9_\-\{\}]{4,80})',
    re.IGNORECASE
)
_PASSWORD_RE = re.compile(
    rb'Password[\x00-\x08]{0,8}([A-Za-z0-9!@#\$%^&\*\-_\.]{6,64})',
    re.IGNORECASE
)
_TAG_RE     = re.compile(
    rb'(?:Tag|LogDirectoryName)[\x00-\x08]{0,8}([A-Za-z0-9_\-\.]{1,64})',
    re.IGNORECASE
)
_VERSION_RE = re.compile(
    rb'Version[\x00-\x08]{0,8}(\d+\.\d+\.\d+\.\d+)',
    re.IGNORECASE
)
